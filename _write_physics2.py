"""
Write the new pool-physics.ts based on the p2.js physics from 7341.e49fb6cc.js.

Physics constants extracted from config object `o` in 7341.e49fb6cc.js (module 14157):
  ball.physics.mass            = 3
  ball.physics.damping         = 0.45   (linear velocity decay per second)
  ball.physics.angularDamping  = 0.9    (angular velocity decay per second)
  ball.physics.sleepSpeedLimit = 5      (their units/s; scaled to px below)
  material.ballHitBall:        restitution = 0.9,  friction = 1.0
  material.ballHitCushion:     restitution = 0.75, friction = 1.0
  config.solverIterations      = 20     (sub-iterations per step)
  stepDt                       = 1/240  (their physics Hz; we use 1/60 with sub-steps)
  ball.radius                  = 11     (their units; ours = 21 px)
  ball.maxBreakAngleModifier   = 0.6
  cueBall.spin.frictionOffset  = 0.3
  cueBall.spin.restitutionOffset = 0.45
  pull.velocity.max            = 2000   (impulse value → velocity = 2000/mass = 667 their-u/s)
  pull.breakVelocity.max       = 3500

Coordinate scaling: their ball radius = 11, ours = 21 px → scale = 21/11 ≈ 1.909
  sleepSpeedLimit_px = 5 * (21/11) ≈ 9.5 px/s
  maxRegularSpeed_px = (2000/3) * (21/11) ≈ 1273 px/s
"""
 ...
 * Source: pull.velocity.max = 2000 impulse → velocity = 2000/mass = 666.7 their-u/s.
 * Scaled: 666.7 * (21/11) ≈ 1273 px/s.
 * We map this to GameConfig.stick.maxPower (800).
 */
const MAX_SHOT_SPEED_PX = (2000 / BALL_MASS) * (R_PX / 11);  // ≈ 1273 px/s
...
export function createInitialBalls(_rackSeed?: number): SimBallInput[] {
    ...
    const rack: Record<number, { x: number; y: number }> = {
        1: rp(0,0), 9: rp(1,0),  2: rp(1,1),
        10: rp(2,0), 8: rp(2,1), 3: rp(2,2),
        11: rp(3,0), 4: rp(3,1), 12: rp(3,2), 5: rp(3,3),
        13: rp(4,0), 6: rp(4,1), 14: rp(4,2), 7: rp(4,3), 15: rp(4,4),
    };
    return [
...
function resolveBallCollision(a: PhysicsDynamicBody, b: PhysicsDynamicBody): boolean {
    ...
    if (vRelN >= 0) return false;  // separating
...
function willHitSegment(
    ...
    // Future position violates the wall plane (within one diameter of it)
    const futureDist = nx * (fpx - seg.start.x) + ny * (fpy - seg.start.y);
    if (futureDist > 0) return false;
...
function advanceSimWorld(
    ...
    for (let sub = 0; sub < SUB_STEPS; sub++) {
        ...
        for (let i = 0; i < n; i++) {
            ...
            const fpx = a.center.x + a.velocity.x * subDt;
            const fpy = a.center.y + a.velocity.y * subDt;
            for (let s = 0; s < CACHED_LINES.length; s++) {
                const norm = CACHED_NORMALS[s];
                if (willHitSegment(a, CACHED_LINES[s], norm, fpx, fpy)) {
                    applyWallBounce(a, norm.nx, norm.ny);
                    onWallHit?.(); break;
                }
            }
        }
...
dest = r'D:/GitHub/acos-games/billiards-8ball/game-client/pool/pool-physics.ts'
